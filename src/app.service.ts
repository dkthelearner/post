...
export class AppService {
    ...
    public async createNewPost(data: CreatePostDto, authUserId: number): Promise<Post> {
        const post = {} as Post;
    ...
    public async getAllPosts(data: { page: number; limit: number; term: string }): Promise<Post[]> {
        let { limit, page } = data;
        if (!page || page === 0) {
            page = 1;
        }
